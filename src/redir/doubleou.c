...
** Last update Sun Jun  5 23:41:35 2016 RIMBAUD
*/
...
int		next(char *name, char **args, char **env, char **path)
{
  int		pid;
  int		statut;
  char		*pathname;
...
int		doubleou(t_list *list, char **to_exec)
{
  char		**path;
  char		**env;
  char		**right_args;
  ...
  if ((path = create_path(env)) == NULL)
    return (-1);
  if (next(to_exec[0], left_args, env, path) == 0)
    return (-1);
  if (next(to_exec[1], right_args, env, path) != 0)
    return (-1);
  return (0);
}
